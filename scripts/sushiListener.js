...
async function sushiListener(){
    const sushiPair = DEX["Pools"]["Sushiswap"];
    const provider = new ethers.providers.WebSocketProvider(
        `wss://eth-mainnet.g.alchemy.com/v2/${process.env.ALCHEMY_SOCKET}`
    )
    console.log("provider is on!")
    const contract = new ethers.Contract(sushiPair, pairABI, provider);
    contract.on("Swap", async (  sender,  amount0In, amount1In,amount0Out,amount1Out,to, event) =>{
        ...
        size = (side=="buy") ? info["amount1In"] : info["amount0In"]
        let Info = {
            dex_name:"sushiswap", 
            price:price,
            size: size,
